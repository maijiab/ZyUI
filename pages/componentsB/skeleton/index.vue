	...
	<view class="zy-demo">
		<view class="zy-demo-wrap">
			...
			<view class="zy-demo-area">
				<view class="container zy-skeleton">
					<view class="userinfo">
						<block>
							<!--u-skeleton-circle 绘制圆形-->
							<image class="userinfo-avatar zy-skeleton-circle" :src="userInfo.avatarUrl"></image>
							<!--u-skeleton-fillet 绘制圆角矩形-->
							<text class="zy-skeleton-fillet">{{userInfo.nickName}}</text>
						</block>
					</view>
					<view style="margin: 20px 0">
						<!--u-skeleton-rect 绘制矩形-->
						<view class="zy-skeleton-rect lists" v-for="(item,index) in lists" :key="index">
							<text>{{item}}</text>
						</view>
					</view>
				</view>
				<!--引用组件-->
				<zy-skeleton bg-color="rgb(250, 250, 250)" :loading="loading" :animation="animation" 
				:el-color="elColor" :border-radius="borderRadius"></zy-skeleton>
			</view>
		</view>
