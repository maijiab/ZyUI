<template>
	<view class="zy-wrap" :style="{
			  opacity: Number(opacity),
			  borderRadius: borderRadius + 'rpx',
			  // 因为time值需要改变,所以不直接用duration值(不能改变父组件prop传过来的值)
			  transition: `opacity ${time / 1000}s ease-in-out`
		   }"
	 :class="'zy-lazy-item-' + elIndex">
		<view :class="'zy-lazy-item-' + elIndex">
			<image :style="{borderRadius: borderRadius + 'rpx', height: imgHeight}" v-if="!isError" class="zy-lazy-item"
			 :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded" @error="loadError" @tap="clickImg"></image>
			<image :style="{borderRadius: borderRadius + 'rpx', height: imgHeight}" class="zy-lazy-item error" v-else :src="errorImg"
...
<script>
	 ...
	 * @example <zy-lazy-load :image="image" :loading-img="loadingImg" :error-img="errorImg"></zy-lazy-load>
	 */
	export default {
		name: 'zy-lazy-load',
		props: {
		...
		mounted() {
			...
			setTimeout(() => {
				...
				}).observe('.zy-lazy-item-' + this.elIndex, (res) => {
