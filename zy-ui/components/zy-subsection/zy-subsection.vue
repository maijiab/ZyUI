<template>
	<view class="zy-subsection" :style="[subsectionStyle]">
		<view class="zy-item zy-line-1" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'zy-item-' + index]"
		 v-for="(item, index) in listInfo" :key="index">
			<view :style="[textStyle(index)]" class="zy-item-text zy-line-1">{{ item.name }}</view>
		</view>
		<view class="zy-item-bg" :style="[itemBarStyle]"></view>
	</view>
</template>
...
<script>
	 ...
	 * @example <zy-subsection active-color="#ff9900"></zy-subsection>
	 */
	export default {
		name: "zy-subsection",
		props: {
		...
		computed: {
			...
			noBorderRight() {
				return index => {
					...
					let classs = '';
					// 不显示右边的边框
					if (index < this.list.length - 1) classs += ' zy-none-border-right';
					// 显示整个组件的左右边圆角
					if (index == 0) classs += ' zy-item-first';
					if (index == this.list.length - 1) classs += ' zy-item-last';
					return classs;
				};
			},
			// 文字的样式
		...
		methods: {
			...
			getTabsInfo() {
				...
				for (let i = 0; i < this.list.length; i++) {
					view.select('.zy-item-' + i).boundingClientRect();
				}
				view.exec(res => {
					if (!res.length) {
						setTimeout(() => {
