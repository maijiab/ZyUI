<template>
	<view class="zy-load-more-wrap" :style="{
		...
		<!-- 加载中和没有更多的状态才显示两边的横线 -->
		<view :class="status == 'loadmore' || status == 'nomore' ? 'zy-more' : ''" class="zy-load-more-inner">
			<zy-loading class="zy-loadmore-icon" :color="iconColor" :mode="iconType == 'circle' ? 'circle' : 'flower'" :show="status == 'loading' && icon"></zy-loading>
			<!-- 如果没有更多的状态下，显示内容为dot（粗点），加载特定样式 -->
			<view :style="[loadTextStyle]" :class="[(status == 'nomore' && isDot == true) ? 'zy-dot-text' : 'zy-more-text']" @tap="loadMore">
				{{ showText }}
			</view>
		</view>
	</view>
...
<script>
	 ...
	 * @example <zy-loadmore :status="status" icon-type="iconType" load-text="loadText" />
	 */
	export default {
		name: "zy-loadmore",
		props: {
