...
<script>
	...
	export default {
		...
		methods: {
			selecterQueryInfo(){
				uni.createSelectorQuery().selectAll('.zy-skeleton').boundingClientRect().exec((res)=>{
					const data_query = res[0][0]; 
					this.windowWidth = data_query.heightwidth;
					this.windowHeight = data_query.height;
					this.top = data_query.bottom - data_query.height;
					this.left = data_query.left;
				})
				// 矩形骨架元素
				this.getRectEls();
				// 圆形骨架元素
			...
			// 矩形元素列表
			getRectEls() {
				uni.createSelectorQuery().selectAll('.zy-skeleton-rect').boundingClientRect().exec((res) => {
					this.RectNodes = res[0];
				});
			},
			// 圆角元素列表
			getFilletEls() {
				uni.createSelectorQuery().selectAll('.zy-skeleton-fillet').boundingClientRect().exec((res) => {
					this.filletNodes = res[0];
				});
			},
			// 圆形元素列表
			getCircleEls() {
				uni.createSelectorQuery().selectAll('.zy-skeleton-circle').boundingClientRect().exec((res) => {
